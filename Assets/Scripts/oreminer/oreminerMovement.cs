...
public class oreminerMovement : MonoBehaviour
{
    [SerializeField]
    public float _speed;
    ...
    private oremineralAwarenessController _mineralAwarenessController;
    ...
    private void Awake()
    {
        ...
        _mineralAwarenessController = GetComponent<oremineralAwarenessController>();
        _depositMineral = GetComponent<depositOreMineral>();
        _rest = GetComponent<rest>();
        gs = GameObject.FindGameObjectWithTag("GameController").GetComponent<gameState>();
        _speed = gs.oreminerSpeed;
    }
